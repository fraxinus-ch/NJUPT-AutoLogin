...
def Notifier(type: int, title: str, msg: str):
    ...
    if type == 0:       #成功
        Notice.show_toast(title=title, msg=msg, icon_path="./ico/Check.ico", duration=3, threaded=True)
    elif type == 1:     #失败
        Notice.show_toast(title=title, msg=msg, icon_path="./ico/Cross.ico", duration=3, threaded=True)
    else:               #其他
        Notice.show_toast(title=title, msg=msg, icon_path="./ico/Tips.ico", duration=3, threaded=True)
